...
class Spark {
  constructor(start, end, range, max) {
    ...
    r.add(start);
    this.walker = new RandomWalker(2, r.x, r.y, new LightningStepGenerator(end.x, end.y, 16, 0.125), 4);
    this.path = [r,];
    this.end = end;
    this.range = range;
    this.max = max || 0;
  ...
  draw(graphics) {
    ...
    graphics.noFill();
    graphics.stroke(128, 128, 255);
    graphics.strokeWeight(4);
    graphics.strokeJoin(BEVEL);
    graphics.beginShape();
    path.forEach((p, i) => {
...
function setup() {
  ...
  for (var i = 0; i < num; i++) {
    sparks.push(new Spark(start, end, 32, 250));
  }
...
function draw() {
  ...
  fill(255);
  sparksGraphics.blendMode(BLEND);
  sparksGraphics.background(0);
  sparksGraphics.blendMode(SCREEN);
  sparks = sparks.map((spark) => {
    ...
    return !spark.finished ? spark : new Spark(start, end, 32, 250);
  });
  blendMode(LIGHTEST);
  //sparksGraphics.filter(BLUR, 2);
  image(sparksGraphics, 0, 0);
  blendMode(BLEND);
  fill(128);
