...
class Jelly {
  constructor(r, g, b, a, w, h, position_buffer, size_min, size_max, size_buffer, smoothing) {
    ...
    for (var arm = 0; arm < 6; arm++) {
      var segs = new SegmentChain();
      for (var i = 0; i < 20; i++) {
        segs.pushHead(16, random(360), width / 2, height / 2);
      }
      this.arms[arm] = segs;
    }
  }
  draw() {
    ...
    var e, d, t, i, s;
    for (t = 0; t < 6; t++) {
      this.arms[t].drawBack((s, e, i, c) => {
        stroke(this.r, this.g, this.b);
        strokeWeight(map(1 / (i * 0.5 + 1), 1, 0, 0.4 * smooth[2], 0));
        noFill();
  ...
  update() {
    ...
    for (var t = 0; t < 6; t++) {
      var i = floor(t / 2);
      var s = 2 * (t % 2) - 1;
      var d = createVector(i == 0 ? s * smooth[2] : 0, i == 1 ? s * smooth[2] : 0, i == 2 ? s * smooth[2] : 0);
      d = q.rotate(d);
      var e = p5.Vector.add(c, d);
      this.arms[t].followBack(e);
    }
  }
}
=======
